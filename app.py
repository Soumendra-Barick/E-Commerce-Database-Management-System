...
def initialize_tables():
    try:
        ...
        conn = connection()
        cur = conn.cursor()
        cur.execute("SELECT setval('customers_id_seq', COALESCE((SELECT MAX(id) FROM customers), 1), false)")
        cur.execute("SELECT setval('products_id_seq', COALESCE((SELECT MAX(id) FROM products), 1), false)")
        cur.execute("SELECT setval('sales_id_seq', COALESCE((SELECT MAX(id) FROM sales), 1), false)")
        cur.execute("SELECT setval('sale_items_id_seq', COALESCE((SELECT MAX(id) FROM sale_items), 1), false)")
        conn.commit()
        cur.close()
        conn.close()
